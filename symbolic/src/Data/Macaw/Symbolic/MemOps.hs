...
newtype GlobalMap sym mem w =
  GlobalMap
  { applyGlobalMap :: 
      forall bak. IsBoolSolver sym bak =>
...
ptrOpNR ::
  ( forall bak. (IsBoolSolver sym bak, 1 <= w) =>
    bak ->
    RegValue sym Mem ->
    NatRepr w ->
    Pred sym -> Pred sym -> Pred sym -> Pred sym ->
...
check :: (IsSymInterface sym, IsBoolSolver sym bak) => bak -> Pred sym -> String -> String -> IO ()
...
doReadMem ::
  (IsSymInterface sym, IsBoolSolver sym bak, Mem.HasLLVMAnn sym, ?memOpts :: Mem.MemOptions) =>
  bak ->
  MemImpl sym ->
...
--     arg6 : Value to write
doWriteMem ::
  (IsSymInterface sym, IsBoolSolver sym bak, Mem.HasLLVMAnn sym) =>
  bak ->
  MemImpl sym ->
  M.AddrWidthRepr ptrW ->
  MemRepr ty ->
