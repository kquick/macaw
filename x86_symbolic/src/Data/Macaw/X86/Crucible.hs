...
import           Lang.Crucible.Backend (IsSymInterface, assert, IsBoolSolver(..))
...
pureSemSymUn
  :: forall sym bak n. (IsSymInterface sym, IsBoolSolver sym bak)
  => Sym sym bak
...
mkPair :: forall sym bak x y
       .  (IsSymInterface sym, IsBoolSolver sym bak, KnownRepr TypeRepr x, KnownRepr TypeRepr y)
       => Sym sym bak
       -> RegValue sym x
       -> RegValue sym y
...
getNumerator :: (IsSymInterface sym, IsBoolSolver sym bak)
                    => (1 <= w, w+1 <= w+w, 1 <= w+w)
                    => NatRepr w
                    -> Sym sym bak
...
getDenominator :: (IsSymInterface sym, IsBoolSolver sym bak)
                    => 1 <= w
                    => NatRepr w
                    -> Sym sym bak
                    -> AtomWrapper (RegEntry sym) (M.BVType w)
...
vecOp1 :: (IsSymInterface sym, IsBoolSolver sym bak, 1 <= c) =>
...
vecOp2 :: (IsSymInterface sym, IsBoolSolver sym bak, 1 <= c) =>
  Sym sym bak {- ^ Simulator -} ->
  Endian      {- ^ How to split-up the bit-vector -} ->
  NatRepr w   {- ^ Total width of the bit-vector -} ->
  NatRepr c   {- ^ Width of individual elements -} ->
...
bitOp2 :: (IsSymInterface sym, IsBoolSolver sym bak) =>
...
unpack ::
  (1 <= c, IsSymInterface sym, IsBoolSolver sym bak) =>
  bak ->
  Endian ->
...
evalE :: (IsSymInterface sym, IsBoolSolver sym bak) =>
...
evalApp :: forall sym bak t. (IsSymInterface sym, IsBoolSolver sym bak) =>
